...
export class DynamicTableDataSource extends DataSource<any> {
  ...
  constructor(private paginator: MatPaginator, private sort: MatSort, @Optional() public dataList?: any[]) {
    super();
    if (!CommonFunctionService.isNullOrUndefined(dataList)) {
      this.data = dataList;
    }
  }
  ...
  connect(): Observable<any[]> {
    ...
    return merge(...dataMutations).pipe(map(() => {
      return this.getPagedData(this.getSortedData(this.data.length > 0 ? [ ...this.data] : []));
    }));
  }
